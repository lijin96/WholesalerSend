...
/**
 * @ClassName: P_Dv_RecoverScan_Z
 * @Description: 总公司吊牌回收
 * @Author: lijin
 * @Date: 2026年6月8日15:15:44
 */
public class P_Dv_RecoverScan_Z extends Activity {
...
//	Thread send ;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.new_p_dv_recoverscan_z);
...
//                    }
                ...
                case 8:
                    ...
                    printbill.print(P_Dv_RecoverScan_Z.this, "         吊牌回收", mark, sacnDataList, sysUserInfo.getUserid());
                    break;
                case 9:
                    MyProgressDialog.close();
                    ShowMessage.Show(mContext, msg.obj.toString());
...
//                    result = accWeb.P_Dv_Scan("P_Dv_RecoverScan_Z", para.toJson());
                    result =accWeb.PostAPIStringInterface("AndroidDv/RecoverScanZ", para.toJson());
//                    Log.d("main", result);
                    ...
                    colors = jsonObject.optString("colors");
                    curcount= jsonObject.optString("goodsNum");
                    lastSuccessBarcode= jsonObject.optString("barcode");


                    if (!jsonObject.optString("refractiveIndex").equals("")){
                        goodsid=goodsid+"  "+jsonObject.optString("refractiveIndex");
                    }

                    if (!jsonObject.optString("diopter").equals("")){
                        colors=colors+" S "+jsonObject.optString("diopter");
                    }
                    if (!jsonObject.optString("astigmatism").equals("")){
                        colors=colors+" C "+jsonObject.optString("astigmatism");
                    }

                    if (Integer.parseInt(nScanCount)<Integer.parseInt(jsonObject.optString("billNum","0"))) {
                        nScanCount=jsonObject.optString("billNum","0");
                    }
            ...
            Thread sendprint = new Thread(new Runnable() {
                ...
                public void run() {
                    try {
                        ...
                        List<Map<String, Object>> slist = SqliteDataHelper.getHelper(getApplicationContext())
                                .QueryDbList(
                        ...
                        if (slist.size() == 0) {
                            ...
                            sacnDataList = SqliteDataHelper.getHelper(getApplicationContext())
                                    .QueryDbList(
                                            "select goodsid,modelm,colors,sum(curcount) as curcount from newscandate GROUP BY goodsid",
                                            null);
                        }
