...
public class SortListMapComparator implements Comparator<Map<String,Object>>{
        ...
        public int compare(Map<String, Object> map1, Map<String, Object> map2) {
            ...
            }else if(sortRule.equals("StockId"))
            {
                //仓库ID不相同比较仓库ID
                if(!map1.get("StockId").toString().equals(map2.get("StockId").toString())){
                    return map1.get("StockId").toString().compareToIgnoreCase(map2.get("StockId").toString());
                }
                //仓库ID相同比较仓库名称
                else{
                    return map1.get("StockName").toString().compareToIgnoreCase(map2.get("StockName").toString());
                }
            }
            ...
            }else if (sortRule.equals("PeiGoodLno")){
                //品检单号（配货单号）不相同比较品检单号（配货单号）
                if(!map1.get("PeiGoodLno").toString().equals(map2.get("PeiGoodLno").toString())){
                    return map1.get("PeiGoodLno").toString().compareToIgnoreCase(map2.get("PeiGoodLno").toString());
                }
                //品检单号（配货单号）相同比较零售商或者分销店
                else{
                    return map1.get("AgentName").toString().compareToIgnoreCase(map2.get("AgentName").toString());
                }
            }
            else if(sortRule.equals("PurOrderNo"))
            {
                //品检单号（配货单号）不相同比较品检单号（配货单号）
                if(!map1.get("PurOrderNo").toString().equals(map2.get("PurOrderNo").toString())){
                    return map1.get("PurOrderNo").toString().compareToIgnoreCase(map2.get("PurOrderNo").toString());
                }
                //品检单号（配货单号）相同比较供应商（代理商）
                else{
                    return map1.get("SupplierName").toString().compareToIgnoreCase(map2.get("SupplierName").toString());
                }
            }
