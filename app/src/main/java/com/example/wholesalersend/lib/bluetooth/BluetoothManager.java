package com.example.wholesalersend.lib.bluetooth;
...
public class BluetoothManager {
    ...
    public boolean initBluetoothServiceAndAutoConnect(Context context, Handler handler) {
        return initBluetoothServiceAndAutoConnect(context, handler, false);
    }
     ...
     * @param context 上下文
     * @param handler 消息处理器
     * @param usePackingDevice true 表示使用装盒入库界面的蓝牙设备，false 表示使用拣货界面的蓝牙设备
     * @return true 表示开始自动连接，false 表示没有保存的设备地址
     */
    public boolean initBluetoothServiceAndAutoConnect(Context context, Handler handler, boolean usePackingDevice) {
        // 如果服务为null或已停止，则重新创建
        ...
        // 尝试自动连接上次保存的设备
        return tryAutoConnectLastDevice(context, usePackingDevice);
    }
    ...
    public boolean tryAutoConnectLastDevice(Context context) {
        return tryAutoConnectLastDevice(context, false);
    }
     ...
     * @return true 表示开始连接，false 表示没有保存的设备地址
     */
    public boolean tryAutoConnectLastDevice(Context context, boolean usePackingDevice) {
     ...
     * 连接蓝牙设备
     * @param device 要连接的设备
     * @return true 表示开始连接，false 表示连接失败（服务未初始化）
     */
